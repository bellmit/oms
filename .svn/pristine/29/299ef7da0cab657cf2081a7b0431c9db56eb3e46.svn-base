...
//				"departId":userInfo.departId
	...
	$scope.getContracts =function(){
		$scope.calRs.sumServiceNum="0";
		$scope.calRs.avgUnitPrice="0";
		$scope.calRs.sumServiceAmount="0";
		/*默认查询在册店铺*/
		...
		$http({
			...
			if(data.code == 1) {
				if(data.data.contractList){
					$scope.calRs.sumServiceNum=data.data.sumServiceNum;
					$scope.calRs.avgUnitPrice=data.data.avgUnitPrice;
					$scope.calRs.sumServiceAmount=data.data.sumServiceAmount;
