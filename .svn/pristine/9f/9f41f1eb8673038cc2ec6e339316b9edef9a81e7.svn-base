qyApp.controller("materiaDet_pubController", ["$scope", "$http", '$compile', "Upload", "$route", "$state", "$stateParams", function($scope, $http, $compile, Upload, $route, $state, $stateParams) {
	...
	function MouseWheelHandler(event) {
		...
		myimage.style.width = Math.max(50, Math.min(1000, myimage.width + (30 * delta))) + "px";
		myimage.style.height = mulriple * Math.max(50, Math.min(1000, myimage.width + (30 * delta))) + "px";
