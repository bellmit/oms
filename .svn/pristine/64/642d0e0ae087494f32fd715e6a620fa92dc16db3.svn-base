qyApp.controller("financeMgtController", function($scope, $http, Upload, $compile, $route, $state, $stateParams) {
var userInfo=JSON.parse(localStorage.userInfo);
	...
	$scope.shiftProductTab=function(productTab){
		$scope.productTab=productTab;
		if(productTab=="1"){
			$scope.search.accountStatus="0";
		}else if(productTab=="2"){
			$scope.search.accountStatus="1";
		}else if(productTab=="3"){
			$scope.search.accountStatus="2";
		}else{
			$scope.search.accountStatus="";
		}
		$scope.getContracts();
	}
