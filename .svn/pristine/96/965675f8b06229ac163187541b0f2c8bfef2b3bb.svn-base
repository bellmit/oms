qyApp.controller("mapDepotDet_pubSingleController", ["$scope", "$http", '$compile', "Upload", "$route", "$state", "$stateParams", function($scope, $http, $compile, Upload, $route, $state, $stateParams) {
	...
	//获取商户公共图片库分页pubSingle
	$scope.createPagePluginPubSingle = function(total, pageSize) {
		$("#pagingPubSingle").empty();
		paging = pagePlugin.createPaging({
			renderTo: 'pagingPubSingle',
			total: total,
			pageSize: pageSize
		});
	...
	$scope.pagePub = function(from, pageSize) {
			var jsonObject = "{\"orgId\":\"" + $scope.orgId + "\",\"pProAttrId\":\"" + $scope.proAttrId + "\",\"proAttrName\":\"" + $scope.proAttrName + "\",\"nub\":\"" + from + "\",\"size\":\"" + pageSize + "\"}";
			$http({
				method: 'post',
			...
			if(data.code == "1") {
				...
				if(data.data.attrList.length > 0) {
					...
					if(Number($scope.countFolderFile) > Number(sizePub)) {
						$scope.createPagePluginPubSingle($scope.countFolderFile, sizePub);
					} else {
						$("#pagingPubSingle").empty();
					}
				} else {
	...
	$scope.getMoveDialog = function() {
		...
		$http({
			...
			if(data.code == "1") {
				$scope.attrListMove = data.data.attrList;
				for(var i = 0; i < $scope.attrListMove.length; i++) {
					if($scope.attrListMove[i].proAttrValue != "" ||
						$scope.attrListMove[i].proAttrId == $scope.proAttrId) {
						$scope.attrListMove.splice(i, 1);
						i--;
					}
				}
				var proArrt = {
					proAttrId: "0",
					proAttrFilePath: "公共图库"
				}
				$scope.attrListMove.push(proArrt);
				$('.shareDialog').hide();
				$('.moveDialog').show();
			} else {
				alertmsg(data.msg, "error")
