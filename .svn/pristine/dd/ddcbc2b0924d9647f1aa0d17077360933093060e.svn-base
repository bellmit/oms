        	...
        	<div class="saleSearch saleSearch2">
        		<form class="form-horizontal">
				  <div class="form-group col-sm-10 mgb0">				  	
				    <label class="nowraps-text control-label">用户名</label>
				    <div class="col-sm-4 pl0">
				      <input type="text" class="form-control mgt5" placeholder="请输入手机号" ng-model="username" />
				    </div>
				    <div class="form-group col-sm-3">
				  	 <button type="submit" class="btn btn-primary" ng-click="getUserListTB()">查询</button>
				    </div>
				  </div>
				  <!--<div class="form-group col-sm-3">				  	
            ...
            <div class="table-responsive PrivTable storesMgt-Table">
                <table class="table table-hover table-striped table-bordered">
                    ...
                    <tr ng-repeat="userlist in userList" ng-if="userlist.roleId==1">
                        <td class="am-ft-yellowgreen">{{userlist.userName}}</td>
                        <td>{{userlist.trueName}}</td>
                        <td>{{userlist.email}}</td>
                        <td style="text-align: left;">{{userlist.departNameTree}}</td>
                        <td>{{userlist.departArea}}</td>
                    ...
                    <tr ng-repeat="userlist in userList" ng-class="{true:'logout'}[userlist.status==2]" ng-if="orgTye==6&&userlist.roleId!=1">
                    ...
                    <tr ng-repeat="userlist in userList" ng-if="userlist.roleId==3 && orgTye==1">
                        ...
                        <td>{{userlist.trueName}}</td>
                        <td>{{userlist.email}}</td>
                        <td style="text-align: left;">{{userlist.departName}}</td>
                        <td>{{userlist.departArea}}</td>
                        <td roleid="{{userlist.roleId}}">{{userlist.roleName}}</td>
                        <td>———</td>
                    </tr>
