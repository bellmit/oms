qyApp.controller("materiaDetController", ["$scope", "$http",'$compile', "Upload", "$route", "$state","$stateParams", function($scope, $http,$compile, Upload, $route, $state,$stateParams) {
	...
	} else if($stateParams.uiParams.orgId == undefined){
		$scope.source = [{level:0,name:"materiaDet"}];
	...
	$scope.getmateriaDetModel = function(obj) {
		...
		var source = {level:0,name:"materiaDet_model"}
		...
		$state.go("materiaDet_model", {
			uiParams: params
		})
	}
	$scope.getmateriaDetPub = function() {
		var source = {level:0,name:"materiaDet_pub"}
		$scope.source.push(source);
		var params={
				"source":$scope.source,
				"type":"1"
