qyApp.controller("materiaDet_pubSingleController", ["$scope", "$http", '$compile', "Upload", "$route", "$state", "$stateParams", function($scope, $http, $compile, Upload, $route, $state, $stateParams) {

	$scope.userInfo = eval('(' + localStorage.userInfo + ')');
	//页面展现，隐藏
	var nub = "0";
	var sizePub = "19";
	if($scope.userInfo.roleId != '3' && $scope.userInfo.roleId != '4') {
		sizePub = "20"
	}
