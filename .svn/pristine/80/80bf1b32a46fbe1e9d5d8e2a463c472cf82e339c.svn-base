qyApp.controller("mapDepotController", ["$scope", "$http",'$compile', "Upload", "$route", "$state","$stateParams", function($scope, $http,$compile, Upload, $route, $state,$stateParams) {
	...
	$scope.sourceArr=[{level:0,name:"mapDepot"}];
	...
	$scope.getmateriaDet = function(obj) {
		var source = {level:0,name:"mapDepotDet"}
