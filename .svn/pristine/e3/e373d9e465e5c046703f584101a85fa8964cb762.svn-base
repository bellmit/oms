qyApp.controller('managecolorController',function($scope,$http,$compile,Upload) {
	 //var brandId = "1";
	 var from=0;
	 var pageSize=5;
	 var total=0;
	 ...
	 $http({
    	 ...
    	 if (data.code == 1) {
	         ...
	         $scope.total=parseInt($scope.items[0].count);
	         $scope.createPagePlugin($scope.total,pageSize);
    	 } else {
    ...
    $scope.createPagePlugin=function(total,pageSize){
     ...
     $scope.add=function(obj){
    	 if(validateForm("colorForm","msg")==true){
	    	 ...
	    	 var jsonObject ="{"+"\"colorName\":\""+colorName+"\",\"colorRgbvalue\":\""+colorRgbvalue+"\",\"pantone\":\""+pantone+"\",\"colorSystem\":\""+colorSystem+"\",\"imagesPath\":\""+imagesPath+"\",\"colorNum\":\""+colorNum+"\",\"brandId\":\""+brandId+"\"}";
	    	 $http({
	             method:'post',
     ...
     $scope.addtr=function(){
    	 ...
    	 addLineColor.push('<td><div class="uploadNav" onmouseover="mouseOver(this)" onmouseout="mouseOut(this)"><input type="file" ngf-select="uploadFiles($files)" /><img alt="请选择图片" src="http://static.qineasy.com/base/images/default_color.png" id="colorImage"><i class="fa fa-close close-del icon-hide" onclick="delImage(this)"></i></div></td>');
...
//  	 addLineColor.push('<td><select class="col-md-12" ng-options="brand.brandId as brand.brandName for brand in brands" ng-model="brand"></select></td>');
     ...
     $scope.getSystem=function(obj){
         $http({
        	 ...
        	 $scope.systemCount= data.data.colorList.length;
...
//  	 addLineColor.push('<td><select class="col-md-12" ng-options="brand.brandId as brand.brandName for brand in brands" ng-model="brand"></select></td>');
     ...
     $scope.addSys=function(obj){
    	 if(validateForm("colorForm","msg")==true){
	    	 ...
	    	 var brandId = $scope.brand;
	    	 var jsonObject ="{"+"\"colorName\":\""+colorName+"\",\"pantone\":\""+pantone+"\",\"colorRgbvalue\":\""+colorRgbvalue+"\",\"colorSystem\":\""+colorSystem+"\",\"imagesPath\":\""+imagesPath+"\",\"colorNum\":\""+colorNum+"\",\"brandId\":\""+brandId+"\"}";
	         $http({
	             method:'post',
	             url:pos+'color/addColor',
	             params:{
