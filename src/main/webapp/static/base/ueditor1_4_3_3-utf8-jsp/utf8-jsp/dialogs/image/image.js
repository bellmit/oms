        ...
        initUploader: function () {
            ...
            function addFile(file) {
                var $li = $('<li id="' + file.id + '">' +
                        ...
                        '<p class="wrap_handles">'+
                        '<a href="javascript:;" class="goLast">&lsaquo;</a>'+
                        '<a href="javascript:;" class="goNext">&rsaquo;</a></p>'+
                        '</li>'),
                ...
                $("#"+file.id+" .goNext").click(function() {
                	console.log($("#"+file.id).next().next());
                    if($("#"+file.id).next()){// 如果存在下一个元素
                		if($("#"+file.id).next().next().length>0){
                			$("#"+file.id).next().after($("#"+file.id));
                		}
                	}                    
                })
                $("#"+file.id+" .goLast").click(function() {
                    if($("#"+file.id).prev()){ // 如果存在上一个元素
                    	$("#"+file.id).prev().before($("#"+file.id));// 就将此元素插到上一个元素之前，从而实现互换顺序
                    }                   
                })
            }
