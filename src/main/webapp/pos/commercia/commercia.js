qyApp.controller("commerciaController", function ($scope,$http,$timeout) {
    var noRecord = '';
    ...
    $scope.AddCommercial=function(){
    	if(validateForm("commerciaForm","msg")==true){
	        ...
	        var jsonObject ="{\"province\":\""+province+"\",\"city\":\""+city+"\",\"district\":\""+district+"\",\"shopAddr\":\""+shopAddr+"\",\"orgId\":\""+orgId+"\",\"shopName\":\""+shopName+"\",\"shopInfo\":\""+shopInfo+"\",\"contacts\":\""+contacts+"\",\"shopTel\":\""+shopTel+"\",\"userId\":\""+userId+"\",\"orgType\":\""+orgType+"\"}";
	        jsonObject = JSON.parse(jsonObject);
	        jsonObject.userPrivList = userPrivList;
...
//修改商户资料
    $scope.Edit=function(obj){
    	if(validateForm("commerciaupdateForm","msg")==true){
	        ...
	        var city="";
	        var district="";
	        if(undefined!=$scope.shopList[0].province){
	        	province = $scope.shopList[0].province;
	        }
	        if(undefined!=$scope.shopList[0].city){
	        	city = $scope.shopList[0].city;
	        }
	        if(undefined!=$scope.shopList[0].district){
	        	district = $scope.shopList[0].district;
	        }
	        var shopAddr = $('.shopAddrDet').val();
	        var shopTel = $('.shopTel').val();
	        var contacts = $('.contacts').val();
