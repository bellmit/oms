qyApp.controller('freightEditController', function($state, $stateParams, $scope, $http, $compile, Upload) {
	...
	$scope.freightTempInfo = $stateParams.uiParams.routeParams;
	$scope.calType = $scope.freightTempInfo.calType;
	$scope.orgId = angular.fromJson(keyParams).orgId;
	$scope.shopProp = "2";
	$scope.orgLists = $scope.freightTempInfo.orgList;
	$scope.chooseOrgIdArr = [];
	angular.forEach($scope.orgLists, function(ele, index) {
		if(ele.orgId == $scope.orgId) {
			$scope.orgLists.splice(index, 1);
		}
	});
	angular.forEach($scope.orgLists, function(ele, index) {
		$scope.chooseOrgIdArr.push(ele.orgId);
	});
	$scope.orgIdStr = "";
	...
	$scope.selectArr = [];
	$scope.logiticTemplateName = $scope.freightTempInfo.logiticTemplateName;
	if($scope.calType == "1") {
		angular.forEach($scope.freightTempInfo.details, function(ele, index) {
			if(ele.locationType == "0") {
				$scope.details_default_count = ele;
			} else {
				$scope.detail_countArr.push(ele);
			}
		})
	} else if($scope.calType == "0") {
		angular.forEach($scope.freightTempInfo.details, function(ele, index) {
			if(ele.locationType == "0") {
				$scope.details_default_weight = ele;
			} else {
				$scope.detail_weightArr.push(ele);
			}
		})
	};
	/*查询微店列表*/
	...
	$scope.loadCity = function(obj, pObj, event, index) {
			...
			$http({
				...
				if(data.code == 1) {
					//					$scope.cityLists = data.data.types;
	...
	$scope.saveFreightTemp = function() {
			...
			var jsonObject = {
				logiticTemplateId: $scope.freightTempInfo.logiticTemplateId,
			...
			$http({
				method: 'post',
				url: pos + 'logiticTemplate/updateLogiticTemplate',
				//								url: 'http://10.0.17.224:8080/pos-api/logiticTemplate/updateLogiticTemplate',
				params: {
					keyParams: getKeyParams(jsonObject, keyParams),
					jsonObject: getJsonObject(jsonObject)
				}
