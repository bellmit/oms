...
//	$scope.data = myDate.getFullYear() + "-" + (1 + myDate.getMonth()) + "-" + myDate.getDate();
	...
	$scope.model.contractType="0";
	$scope.selectedSite='0';
	if($stateParams.uiParams.continueType=="1"){
...
//			}
		...
		var jsonObject={
			contractType:$scope.model.contractType,
...
//					$scope.goback();
	...
	$scope.getDepartList = function(departArea,objClass){
		if(departArea==""){
			...
			$('.'+objClass+'1').remove();
			var jsonObject = "{\"pDepartId\":\""+0+"\",\"departArea\":\""+departArea+"\",\"productType\":\"商务\"}";
			$http({
				method: 'post',
	...
	$scope.sortIdChange = function(id, type){
		$scope.departId=id;
		var jsonObject = "{\"pDepartId\":\""+id+"\",\"productType\":\"商务\"}";
		$http({
			method: 'post',
			url: cas + 'depart/getDepartList',
			params: {
...
function sortIdChange(obj){
	...
	var jsonObject = "{\"pDepartId\":\""+departId+"\",\"productType\":\"商务\"}";
